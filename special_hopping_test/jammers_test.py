...
class FastNoiseSource:
    ...
    def __init__(self, Fs, bandwidth, duration=1.0):
        ...
        # 2. FFT
        spec = np.fft.rfft(n)
        # 3. Low-pass to +/- bandwidth/2. After cosine modulation this gives
        # one RF interference band whose total width is exactly ``bandwidth``.
        if bandwidth > 0:
...
class IndiscriminateJammer:
    def __init__(self, Fs, sweep_config=None, comb_config=None, 
        ...
        self.c_sub_interval = float(self.comb_config.get('sub_interval', 50000.0))
        if not np.isclose(self.c_bw, self.c_sub_interval):
            raise ValueError(
                "The special comb jammer bandwidth must equal one channel width."
            )

        default_phase_channels = (
            [0, 2, 4, 6, 8, 10, 12, 14],
            [1, 3, 5, 7, 9, 11, 13, 15],
        )
        phase_channels = self.comb_config.get(
            'phase_channels', default_phase_channels
        )
        if len(phase_channels) != 2:
            raise ValueError("comb phase_channels must contain exactly two groups")

        parsed_phase_channels = []
        for phase, channels in enumerate(phase_channels):
            channel_array = np.asarray(channels, dtype=np.int64)
            if channel_array.ndim != 1 or channel_array.size == 0:
                raise ValueError(
                    f"comb phase {phase} channels must be a non-empty vector"
                )
            if np.any(channel_array < 0):
                raise ValueError("comb channel indices must be non-negative")
            if len(np.unique(channel_array)) != len(channel_array):
                raise ValueError(
                    f"comb phase {phase} contains duplicate channel indices"
                )
            parsed_phase_channels.append(channel_array)
        self.comb_phase_channels = tuple(parsed_phase_channels)
    ...
    def generate(self, t, Startfre, Endfre):
        ...
        if self.mode == 'comb' or self.mode == 'both':
            ...
            sub_interval = self.c_sub_interval
            target_indices = self.comb_phase_channels[self.comb_phase]
