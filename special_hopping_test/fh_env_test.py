...
def save_psd_waterfall_plot(waterfall_db, output_path, plot_title=""):
    """Save a waterfall using the same visual style as the original env."""
    if waterfall_db.size == 0:
        return
...
class FHSSQPSKEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.mseq_states = generate_mseq_states(n_bits=mseq_nbits,
                                                ...
                                                taps=mseq_taps,
                                                seed=mseq_seed)
        special_patterns = np.asarray(
            settings.BLOCK_HOP_PATTERNS, dtype=np.int64
        )
        if self.num_channels != 20:
            raise ValueError(
                "The special hopping test requires exactly 20 channels."
            )
        if not np.isclose(self.base_hoprate, 100.0):
            raise ValueError(
                "The special hopping test requires a fixed 100 Hz hop rate."
            )
        if special_patterns.shape != (10, 10):
            raise ValueError(
                "BLOCK_HOP_PATTERNS must contain ten blocks of ten hops."
            )
        if np.any(special_patterns < 0) or np.any(
            special_patterns >= self.num_channels
        ):
            raise ValueError("Special hopping channels must be in [0, 19].")

        self.special_block_patterns = special_patterns.copy()
        self.mseq_channels = special_patterns.reshape(-1).copy()
        self._mseq_ptr = 0
    ...
    def configure_psd_capture(self, steps, output_dir):
        """Configure one-based environment steps whose waterfalls are saved."""
        parsed_steps = {int(step) for step in steps}
        if any(step <= 0 for step in parsed_steps):
            raise ValueError("PSD capture steps must be positive one-based values.")
        self.capture_steps = parsed_steps
        self.capture_output_dir = os.path.abspath(output_dir)
    ...
    def step(self, action=None):
        if self.reset_mseq_each_step:
            # Make every step start from the exact same 10-block base template.
            self._mseq_ptr = 0

        step_number = self.current_step + 1
        comb_phase = (
            int(self.sweep.comb_phase)
            if self.enable_sweep and self.sweep is not None
            else None
        )
        capture_dir = None
        if (
            step_number in self.capture_steps
            and self.capture_output_dir is not None
        ):
            capture_dir = os.path.join(
                self.capture_output_dir, f"step_{step_number:04d}"
            )
            save_psd_waterfall_plot(
                np.asarray(self.state),
                os.path.join(capture_dir, "obs.png"),
                plot_title=f"Step {step_number} Input Observation",
            )
        ...
        for b in range(10):
            ...
            # ----------------------------------------------------------------
            base_hop_seq_block = self._get_block_hopseq(hops_per_block, 0)
            expected_base = self.special_block_patterns[b]
            if not np.array_equal(base_hop_seq_block, expected_base):
                raise RuntimeError(
                    f"Step {step_number}, block {b + 1}: base hopping "
                    "template did not reset correctly."
                )
            off_int = int(np.round(offsets_action[b])) % self.num_channels
            hop_seq_block = (base_hop_seq_block + off_int) % self.num_channels
            base_hop_sequences.append(base_hop_seq_block.astype(int).tolist())
            ...
            if capture_dir is not None:
                block_waterfall = compute_psd_waterfall(
                    rx_real,
                    fs=self.Fs,
                    f_start=self.Startfre,
                    f_end=self.Endfre,
                ...
                save_psd_waterfall_plot(
                    block_waterfall,
                    os.path.join(capture_dir, f"block_{b + 1:02d}.png"),
                    plot_title=f"Step {step_number} - Block {b + 1}",
                )
            elif self.debug_plot_psd:
        ...
        self.last_info = {
            ...
            "comb_phase": comb_phase,
            "base_hop_sequences": base_hop_sequences,
